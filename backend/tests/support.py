"""Test doubles: real PNG bytes, canned extractions, and a deterministic fake
orchestrator agent that drives the DB directly (no in-process tools / no live
providers). Mirrors the MCP-only flow: extraction is canned, PO read/upsert and
decision persistence go through app.db.repository, and the handler reads the
decision back from the DB."""
...
from app.db.repository import get_purchase_order_by_number, persist_decision, upsert_purchase_order
...
_SUBTOTAL = 55615.0
_TAX = round(_SUBTOTAL * 0.09125, 2)
_TOTAL = round(_SUBTOTAL + _TAX, 2)


def invoice_matching_po_54872(**overrides) -> ExtractedInvoice:
    inv = ExtractedInvoice(
        invoice_number="INV-9001", po_number="PO-54872", invoice_date="2025-05-01",
        currency="USD", subtotal=_SUBTOTAL, tax_amount=_TAX, discount_amount=0.0,
        shipping_charges=0.0, total_amount=_TOTAL,
        ...
        line_items=[InvoiceLineItem(description=d, quantity=q, unit_price=u, tax_rate=0.09125, total_price=t)
...
def invoice_for_new_po(po_number="PO-NEW-1") -> ExtractedInvoice:
    return ExtractedInvoice(
        invoice_number="INV-NEW-1", po_number=po_number, invoice_date="2025-06-01",
        currency="USD", subtotal=100.0, tax_amount=round(100.0 * 0.09125, 2),
        discount_amount=0.0, shipping_charges=0.0, total_amount=round(100.0 + 100.0 * 0.09125, 2),
        vendor=VendorInfo(name="TechCore Solutions LLC", tax_id="99-9"),
        line_items=[InvoiceLineItem(description="Widget", quantity=2, unit_price=50.0, tax_rate=0.09125, total_price=100.0)],
    )


def uploaded_po(po_number="PO-NEW-1") -> PurchaseOrder:
    return PurchaseOrder(
        po_number=po_number, currency="USD", subtotal=100.0,
...
# --- Reference check logic (prod uses the LLM prompt) -------------------- #
...
def _run_invoice_checks(inv: ExtractedInvoice) -> list[Check]:
    ...
    missing = [f for f, v in {
        ...
        "subtotal": inv.subtotal, "total_amount": inv.total_amount,
        "currency": inv.currency, "vendor_name": inv.vendor.name,
    }.items() if v in (None, "")]
    checks.append(Check(id="mandatory_fields", status="pass" if not missing else "fail",
                        detail="all present" if not missing else f"missing: {missing}"))
    cur_ok = (inv.currency or "").upper() in s.supported_currencies
    checks.append(Check(id="currency", status="pass" if cur_ok else "fail", detail=f"{inv.currency}"))
    line_bad = [li.description for li in inv.line_items if abs(li.quantity * li.unit_price - li.total_price) > s.line_tolerance]
...
def _run_po_checks(inv: ExtractedInvoice, po: PurchaseOrder | None, skip_reason: str) -> list[Check]:
    if po is None:
        return [Check(id="po_vendor_match", status="skipped", detail=skip_reason),
    ...
    checks = [Check(id="po_vendor_match", status="pass" if confidence >= s.vendor_match_threshold else "fail",
                    confidence=confidence, rationale=f"name_match={name_match} tax_match={tax_match}")]
    po_by_desc = {(li.description or "").lower(): li for li in po.line_items}
    bad = []
    for li in inv.line_items:
        m = po_by_desc.get((li.description or "").lower())
        if m is None:
            bad.append(li.description); continue
        qty_ok = m.quantity == 0 or abs(m.quantity - li.quantity) / abs(m.quantity) <= s.po_qty_tolerance
        price_ok = m.total_price == 0 or abs(m.total_price - li.total_price) / abs(m.total_price) <= s.po_total_price_tolerance
...
class FakeAgent:
    """Replays the turn deterministically against the DB (no tools, no providers)."""

    def __init__(self, conversation_id: str, extraction_map: dict):
        self.conversation_id = conversation_id
        self.extraction = extraction_map
        self.last_invoice: ExtractedInvoice | None = None

    def _process(self, prompt: str) -> str:
        has_invoice = "invoice=invoice_1" in prompt
        has_po = "po=po_1" in prompt
        if not has_invoice and self.last_invoice is None and not has_po:
            return "How can I help with your invoice?"
        if has_invoice:
            self.last_invoice = self.extraction["invoice"]
        if self.last_invoice is None:
            return "Please upload an invoice to process."
        inv = self.last_invoice

        po: PurchaseOrder | None = None
        skip_reason = "no PO number"
        if has_po:
            po = self.extraction["purchase_order"]
            with session_scope() as s:
                upsert_purchase_order(s, po)
            po = po.model_copy(update={"source": "uploaded"})
        elif inv.po_number:
            with session_scope() as s:
                found = get_purchase_order_by_number(s, inv.po_number)
            if found:
                po = found
            else:
        ...
        with session_scope() as s:
            rid = persist_decision(s, decision, self.conversation_id)
        return f"{verdict}. Recorded as {rid}."

    async def invoke_async(self, prompt: str) -> str:
        return self._process(prompt)

    async def stream_async(self, prompt: str):
        if "invoice=invoice_1" in prompt:
            yield {"current_tool_use": {"toolUseId": "t1", "name": "extract_document"}}
            yield {"message": {"content": [
                {"toolResult": {"toolUseId": "t1", "status": "success", "content": [{"text": "{\"invoice_number\": \"...\"}"}]}}
            ]}}
...
def install_fake_agent(monkeypatch, extraction_map: dict):
    """Wire the AGENT_FACTORY seam with a deterministic DB-backed fake agent."""
    ...
    def factory(conversation_id: str):
        if conversation_id not in agents:
            agents[conversation_id] = FakeAgent(conversation_id, extraction_map)
