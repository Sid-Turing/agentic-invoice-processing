...
- **FR-019**: The system MUST treat the PO database as read-only reference data and MUST NOT persist submitted invoices or decisions; there is no durable store of past submissions.
- **FR-020**: The system MUST operate as a standalone service requiring only the uploaded invoice, a reachable PO reference database, and configured model credentials — no email service and no frontend are required to run it.
...
- **SEC-004**: The service's access to the PO reference database MUST be read-only; the feature performs no writes to purchase-order, vendor, or line-item data.

### Key Entities *(include if feature involves data)*

- **Submission**: One processing request — a single invoice document. Transient; not stored.
...
- **SC-007**: Transient failures — provider errors and database unavailability — are reported as retryable and are never reported as a business `NEEDS_REVIEW` or as "PO not found".
...
- **Purchase orders come from a database**: The user uploads only the invoice. Purchase orders (with their vendors and line items) already exist in a **read-only PO reference database**, queried by the PO number extracted from the invoice. The exact database engine and access layer are plan-level decisions.
- **Database seed data**: The reference database is expected to be seeded from the existing project data — `purchase_orders_data.csv`, `po_vendors_data.csv`, and `purchase_order_line_items_data.csv` — which define the PO, vendor, and PO-line-item shapes. Seeding/loading itself is operational setup, not part of this feature's runtime behavior.
